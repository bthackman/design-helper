...
| **Project Window** | **Not a Studio.** A Studio generates and steers geometry for one phase. The Window reads the whole project and steers nothing. It *hosts* Studios; it doesn't replace them. Its Massing phase tab carries its own Schematic/Room-massing sub-toggle, opening the relevant mode of whichever of the three rows above actually exists for the project. |
...
| **Room massing (live)** — *massing* | A third, distinct topbar mode in Massing Studio, alongside Stage 1/Stage 2 above — **not** a form of promotion. One 3D box per Test-Fit room (never grouped by type), fed through the same `drawSection()`/cap meter live, with no click and no write to `state` ever — pure, reversible-by-construction read. Façade/`effLoss`/glazedPct/driver-proxies/trade-space are hidden in this mode, not faked, for the same reason Stage 2 gates them (`nVol`/facade stop meaning what they mean at room grain). Only real once Massing Studio and Test-Fit Studio share a runtime (`Design-Studio.html`); falls back to Test-Fit's own standalone "3D room massing (standalone)" tab otherwise (no section/sun-study/cap-meter there). See `phase-3-massing.md`'s "Room massing (live)" section. |
