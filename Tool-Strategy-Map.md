...
1. **Decide OQ-18 (single- vs two-storey), then close Phase 4** — pilot space planning is tested + paused; pick the scheme (roster favours office-at-grade), verify program areas vs the course brief, loop the winner back to the **Massing Studio** (plate grew to ~120 m² on one level), then fold the P4 method into `SKILL.md`. A **Test-Fit Studio** (mirroring the Massing Studio, *place → steer*, same client-side/model-in-loop cost split, with a live doorway/clearance check) is still the tooling target.
