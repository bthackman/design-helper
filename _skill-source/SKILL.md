...
**Stage 1 (siting, LOD 100) → Stage 2 (articulation, LOD 200), per volume, not per project or page.** Once phase 4 places real rooms against a volume, it can be **promoted** — a "Reshape to fit" box-grow (never one-box-per-room) that resizes/repositions the volume to the rooms' real bbox, previewable and undoable like any slider edit; a promoted volume gets a badge next to its own entry in the picker (never a page-level mode toggle), and `facade`/`effLoss`/`glazedPct`/D3 render tagged `[schematic-only — see Stage 2]` for it while `coverage`/`nVol` keep computing with a caption. Full mechanism, metric-gating rule, and the phase-4 trigger: `references/phase-3-massing.md` / `references/phase-4-space-planning.md`.
...
5. **"Room massing (live)" topbar mode** — a third, distinct mode (not Stage 2/promotion above): one live 3D box per Test-Fit room, never grouped by type, fed through the same `drawSection()`/cap meter, live, with no click and nothing ever written to `state`. Shows only the honest subset of instruments (cap meter, coverage, volume) — façade/glazed/driver-proxies/trade-space are hidden, not faked, at this grain. Only real once Massing Studio and Test-Fit Studio share a generated runtime (`Design-Studio.html`, built by `_UI/build-design-studio.js` — see `references/phase-3-massing.md`'s "The merged runtime" section); falls back to Test-Fit Studio's own standalone room-massing tab otherwise. Build the mode toggle in regardless of whether the merge exists yet for this project.
